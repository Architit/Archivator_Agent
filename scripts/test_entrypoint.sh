...
case "${1:---all}" in
  ...
  --transport)
    ...
    ;;
  --flow-control)
    "$PYTEST_BIN" -q tests -k "phase_e_flow_control_archivator or test_phase_b_patch_runtime_contract"
    ;;
  --patch-runtime)
  ...
  --ci)
    ...
    ;;
  *)
    echo "usage: $0 [--all|--unit-only|--integration|--governance|--control|--memory|--transport|--flow-control|--patch-runtime|--ci]"
    exit 2
    ;;
esac
